...
app.post("/bettermomentum/addserver", async (req, res) => {
    try {
        ...
        const existingServer = await GameServers.findOne({ 
            ip, 
            port, 
            playlist 
        });
        ...
        if (existingServer) {
            return res.status(409).json({ 
                error: "Server already exists. Cannot register duplicate server. please remove the old one first" 
            });
        }
...
app.post("/bettermomentum/heartbeat", async (req, res) => {
    try {
        ...
        const server = await GameServers.findOne({ 
        ...
        if (!server) {
            return res.status(404).json({ error: "Server not found with provided serverKey" });
        }
...
app.post("/bettermomentum/removeserver", async (req, res) => {
    try {
        ...
        if (!server) {
            ...
            return res.status(404).json({ error: "Server not found or invalid serverKey" });
...
app.get("/bettermomentum/serverlist", async (req, res) => {
